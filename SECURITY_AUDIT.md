...
> **Least-privilege check — action required:** A credential-safe `SHOW GRANTS FOR CURRENT_USER()` inspection found that the current managed application connection uses a root-level account with global privileges and `GRANT OPTION`. The account identifier is intentionally omitted from this report. This **fails** the least-privilege requirement and must not be treated as verified. Before production use, create a dedicated application database user restricted to the TRYCAT schema and the minimum required data operations; use a separately controlled migration account for DDL, then update the managed `DATABASE_URL` through the hosting platform’s secret system.
